...
if(preg_match('/^supportCat(.*)/',$data,$match)){
    delMessage();
    sendMessage("💠لطفا موضوع تیکت را ارسال کنید! (کوتاه)", $cancelKey);
...
if($data == "managePanel" and (($from_id == $admin || $userInfo['isAdmin'] == true))){
    ...
    setUser();
    $msg = "👤 عزیزم به بخش مدیریت خوشومدی 
...
if($data == 'reciveApplications') {
    ...
    $keyboard[] = ['text'=>$buttonValues['back_to_main'],'callback_data'=>"mainMenu"];
    $keyboard = array_chunk($keyboard,1); 
    editText($message_id, "📌 شما میتوانید راهنمای اتصال به سرویس را از طریق کانال رسمی ما دنبال کنید و همچنین از دکمه های زیر میتوانید برنامه های مورد نیاز هر سیستم عامل را دانلود کنید

💢 برنامه پیشنهادی V2rayNG  

🔗 با کلیک بر روی نام برنامه، می توانید لینک دانلود را دریافت کنید ", json_encode(['inline_keyboard'=>$keyboard]));
}
